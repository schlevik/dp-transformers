...
def main(args: Arguments):
    ...
    # Load model
    model = transformers.AutoModelForCausalLM.from_pretrained(args.script_args.model_name, attn_implementation='flash_attention_2', torch_dtype=torch.bfloat16)
    ...
    # mean_tok_emb = model.wte.weight.data.mean(dim=0)
    ...
    num_samples = 5000 # len(dataset['train'])
    privacy_args.target_delta = 1.0/(num_samples*math.log(num_samples))
    logger.info(f"Privacy parameters {privacy_args}, num_samples={num_samples}, delta={privacy_args.target_delta}")
    # assert False
    model = model.cuda()
    model.train()
