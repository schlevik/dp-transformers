...
for i in {4..99}; do
    dataset_file="/data/yidan/MIA/data/cls/psytar/D_sample/sampled_private_datasets/dataset_${i}.jsonl"
    echo "dataset_file: $dataset_file"
    if [ -f "$dataset_file" ]; then
        echo "Processing $dataset_file"
        for epsilon in 2; do
            echo "Processing $dataset_file with epsilon $epsilon"
            output_dir="/data/$user_name/mia_${dataset_name}/epsilon_${epsilon}/dataset_${i}"
            mkdir -p "$output_dir"
        ...
        #     mkdir -p "$output_dir"
        #     CUDA_VISIBLE_DEVICES=1,2 python3 -m torch.distributed.run --master_port 29501 --nproc_per_node 2 fine-tune-nodp.py \
        #         --output_dir "$output_dir" \
        #         --model_name meta-llama/Llama-3.2-1B \
        #         --train_file "$dataset_file" \
        #         --sequence_len $sequence_len \
