...
for i in 15; do
    ...
    if [ -f "$dataset_file" ]; then
        ...
        for epsilon in 0.5 1; do
            echo "Processing $dataset_file with epsilon $epsilon"
            output_dir="/data/$user_name/$dataset_name/${i}/${epsilon}"
            mkdir -p "$output_dir"
            CUDA_VISIBLE_DEVICES=4,7 python3 -m torch.distributed.run --master_port 29502 --nproc_per_node 2 fine-tune-dp.py \
