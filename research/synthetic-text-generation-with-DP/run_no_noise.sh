...
for i in {82..99}; do
    ...
    if [ -f "$dataset_file" ]; then
        ...
        #     mkdir -p "$output_dir"
        #     CUDA_VISIBLE_DEVICES=3,4 python3 -m torch.distributed.run --nproc_per_node 2 fine-tune-dp.py \
        ...
        for epsilon in 0; do
            ...
            output_dir="/data/$user_name/$dataset_name/${i}/${epsilon}"
            mkdir -p "$output_dir"
            CUDA_VISIBLE_DEVICES=0,7 python3 -m torch.distributed.run --master_port 29502 --nproc_per_node 2 fine-tune-nodp.py \
                --output_dir "$output_dir" \
                --model_name meta-llama/Llama-3.2-1B \
                --train_file "$dataset_file" \
                --sequence_len $sequence_len \
