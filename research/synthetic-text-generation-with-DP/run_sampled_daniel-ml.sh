...
for i in {0..99}; do
    ...
    if [ -f "$dataset_file" ]; then
        ...
        for epsilon in 0; do
            ...
            mkdir -p "$output_dir"
            CUDA_VISIBLE_DEVICES=4,5,6,7 python3 -m torch.distributed.run --nproc_per_node 4 fine-tune-nodp.py \
        ...
        #     output_dir="/data/srini/$dataset_name/${i}/${epsilon}"
        #     mkdir -p "$output_dir"
        #     CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 python3 -m torch.distributed.run --nproc_per_node 6 fine-tune-dp.py \
        #         --output_dir "$output_dir" \
        #         --model_name meta-llama/Llama-3.2-1B \
        #         --train_file "$dataset_file" \
        #         --sequence_len 256 \
